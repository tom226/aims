...
function CreateModal({ onClose, onSaved }) {
  ...
  useEffect(() => {
    api.get('/suppliers?limit=200').then(({ data }) => setSuppliers(data.data || data));
  ...
  function updateItem(i, key, val) {
    ...
    if (key === 'productId') {
      ...
      if (p) { next[i].unitPrice = p.costPrice; next[i].taxRate = p.taxRate; }
    }
    setItems(next);
  }
  ...
  const total = items.reduce((s, it) => {
    ...
    return s + sub + sub * (parseFloat(it.taxRate) || 0) / 100;
  }, 0);
  ...
  async function save() {
    ...
    try {
      await api.post('/purchase-returns', {
        supplierId, returnDate: date, reason,
        items: valid.map(it => ({
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b dark:border-gray-700 flex items-center justify-between">
          <h2 className="font-semibold text-lg dark:text-white">New Purchase Return / Debit Note</h2>
...
export default function PurchaseReturns() {
  ...
  async function action(id, what) {
    ...
    try {
      await api.post(`/purchase-returns/${id}/${what.toLowerCase()}`);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold dark:text-white">Purchase Returns / Debit Notes</h1>
      ...
      <div className="card p-0 overflow-hidden">
        {loading ? <div className="p-8 text-center text-gray-400">Loading...</div> : list.length === 0 ? (
          <EmptyState icon={ReceiptRefundIcon} title="No purchase returns" description="Issue debit notes for returned purchases." action={<button onClick={() => setShow(true)} className="btn-primary">New Return</button>} />
        ) : (
